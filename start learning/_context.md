...
## Next Up
**Module 5 — Designing Custom Workflows with LangGraph** (4h 20m, 7 sub-topics — the core module): Building the Workflow · Persistence · Time Travel · Tool · Interrupts · Memory · LangGraph APIs (Functional and Graph API). This is where the Research Assistant grows to v0.3+: the backward edge (turning v0.1's chain into a real agent loop), checkpointer, time travel, real tool nodes, interrupt for human approval, and long-term memory. Start with Module 5.1 — Building the Workflow (40m): the `add_conditional_edges` call that replaces `add_edge("evaluate", END)` and finally makes `evaluate`'s verdict matter.
